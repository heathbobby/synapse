...
| ADR-0011 | accepted | Internal foundation delivery mode is CLI-assisted orchestration using the existing framework. | The sponsor selected CLI-assisted delivery for the repository foundation. This keeps artifact generation executable through `orchestration-framework/cli.py`, task cards, memos, and committed docs while leaving Synapse product runtime decisions to product MVPs. | Sponsor decision 2026-05-03; `docs/work_items/INDEX.md`; `docs/planning/EXECUTION_ORCHESTRATION.md`; `docs/product/MVP_STRATEGY.md` | 2026-05-03 |
| ADR-0012 | accepted | Use the orchestration framework itself as the internal foundation domain/initiative. | The sponsor selected the orchestration framework as the first internal domain. This provides source-backed process scaffolding while `docs/product/MVP_STRATEGY.md` defines separate Synapse product MVPs. | Sponsor decision 2026-05-03; `docs/work_items/DEPENDENCY_MAP.md`; `docs/product/MVP_STRATEGY.md` | 2026-05-03 |
| ADR-0013 | accepted | Use Markdown-first canonical metadata for MVP1, with machine-readable extraction deferred. | Markdown tables and structured headings are already used across canonical docs and are sufficient for CLI-assisted MVP1. Machine-readable front matter or schemas should be introduced only when E03 validators demonstrate a concrete need. | `docs/work_items/TECHNICAL_REFINEMENT_GATES.md`; `docs/standards/AI_AGENT_STANDARDS.md` | 2026-05-03 |
| ADR-0014 | accepted | Initial deterministic validators target file presence, required sections, trace IDs, ID format, source immutability, and completion-signal format. | This scope is feasible for CLI-assisted MVP1 and avoids unsupported runtime commitments while improving over the framework validator limitation observed during concept extraction and backlog refinement. | `docs/planning/EXECUTION_ORCHESTRATION.md`; `docs/work_items/TECHNICAL_REFINEMENT_GATES.md` | 2026-05-03 |

## Open Architecture Decisions

| ID | Decision Needed | Why It Matters | Current Guidance | Source |
| --- | --- | --- | --- | --- |
| OAD-0001 | MVP scope after CLI-assisted foundation | Component depth, adapter priorities, and approval policies beyond MVP1 depend on later target workflows. | The internal foundation uses CLI-assisted orchestration and the orchestration framework as its domain; Synapse product MVPs are defined separately in `docs/product/MVP_STRATEGY.md`. | ADR-0011; ADR-0012; `work_items/synapse-product-brief.md` |
...
- CLI-assisted orchestration is sufficient for MVP1 foundation work before a runtime-backed Synapse product slice is designed.
- The orchestration framework is an adequate first internal domain for reusable workflow/task-packet refinement.
